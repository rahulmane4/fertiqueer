...
const Meditation = () => {
  ...
  const { sound, isPlaying, songs, position, duration, currentTrackIndex } = state;
  ...
  const loadSound = async (trackIndex: number) => {
    ...
    const { sound: newSound } = await Audio.Sound.createAsync(songs[trackIndex].url);
  ...
  const nextSong = () => {
    const nextTrack = (currentTrackIndex + 1) % songs.length;
    loadSound(nextTrack);
    sliderRef.current?.scrollToIndex({ index: nextTrack });
  };

  const previousSong = () => {
    const prevTrack = (currentTrackIndex - 1 + songs.length) % songs.length;
    loadSound(prevTrack);
        ...
        renderItem={({ item, index }) => (
          <View style={styles.listContainer}>
            ...
            <View style={styles.carousel}>
              <Image
                source={require("./assets/meditation_bg.png")}
